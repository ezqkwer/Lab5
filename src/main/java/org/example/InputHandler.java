...
public class InputHandler {
    ...
    //создание и инициализация команд
    //ключ-название команды; значение - объект типа Command
    ...
    //интерактивное взаимодействия с пользователем
    public void runInteractiveMode() {
        String commandLine;
        do {
            System.out.print("> ");
            commandLine = inputReader.getScanner().nextLine().trim();
            String[] parts = commandLine.split("\\s+", 2); // разделяет введенную команду на имя и аргумент команды
            String commandName = parts[0].toLowerCase();
            String argument = parts.length > 1 ? parts[1] : null;

            Command command = commandRegistry.get(commandName);  //получение объекта по имени
            if (command != null) {
