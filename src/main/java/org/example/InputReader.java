...
public class InputReader {
    ...
    public String readString(String prompt, boolean nullable) { //nullable -- разрешен ли null
        while (true) {
    ...
    public int readInt(String prompt, boolean nullable, int min, int max) {
        while (true) {
            ...
            if (nullable && valueStr == null) {
                return Integer.MIN_VALUE; //  специальное значение о null для int (-2147483648)
            }
            try {
    ...
    //extends Enum<E>: ограничение для типа E (E должен быть enum)
    //Возвращает значение типа E, то есть значение того enum типа, который был запрошен
    //Объект типа Class, который представляет собой класс типа E
    ...
    public Human readHuman() {
        Long age = null; // Инициализируем age в null, чтобы использовать в цикле

        while (age == null) {
            age = readLong("Введите возраст губернатора (должен быть > 0): ", false, 1L, Long.MAX_VALUE); // nullable = false

            if (age == null) { // на всякий случай
                System.out.println("Поле 'Возраст губернатора' не может быть null.");
            }
        }
